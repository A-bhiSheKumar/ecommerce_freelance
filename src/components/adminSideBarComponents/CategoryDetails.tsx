...
const CategoryDetails: React.FC = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const rawList: ICategory[] = Array.isArray(response)
            ...
            {filtered.map((product) => {
              return (
                <div
                  ...
                  onClick={() =>
                    navigate(
                      `/home/product?category=${encodeURIComponent(
                        product.id
                      )}&categoryName=${encodeURIComponent(product.name)}`
                    )
                  }
                  className="group rounded-xl bg-white/5 backdrop-blur-md border border-white/10 hover:border-white/20 transition shadow-sm hover:shadow-md overflow-hidden cursor-pointer"
                >
                  {/* Image / Placeholder */}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation(); // ✅ prevent card click
                            handleDeleteCategory(product.id);
                          }}
